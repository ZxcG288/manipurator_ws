...
class PublisherNodeClass(Node):
    ...
    def __init__(self):
        ...
        self.topicNameFrames = '/image/compressed'  # Updated topic name
        self.queueSize = 15
        self.publisher = self.create_publisher(CompressedImage, self.topicNameFrames, self.queueSize)
        self.periodCommunication =  0.033 #0.067 at 15Hz # 0.033 at 30Hz # 0.1 at 10Hz #0.143 at 7Hz  #FPS
    ...
    def timer_callback(self):
        ...
        if success:
            ...
            # บีบอัดภาพเป็น JPEG
            encode_param = [int(cv2.IMWRITE_JPEG_QUALITY), 80]  # 50 = คุณภาพต่ำ, 100 = คุณภาพสูง
            try:
